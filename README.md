...
$status = ($idade<18) ? "Criança" : ($idade<60) ? "Adulto" :
"Idoso";
```
...
Um laço de repetição faz com que um bloco de códigos rode várias vezes, até que uma condição mande para.

- O Laço `while`(Enquanto)

Ele verifica se a condição é verdadeira AMNTES de entrar no laço. Ideal quando você não sabe quantas vezes vai rodar o laço.
...
A diferença é que ele execita o bloco pelo menos uma vez, mesmo que a condição seja falsa desde o início, pois ele só pergunta no final.
...
>obs: Uso de Ideia do ```do-while```, Menus de sistema ou sistema de solicitações de dados, sistemas interativos;
...
//usar uma função nativa para a substituição de parte de texto ==> str_replace
...
#### Prinpais Funções Nativas (Mais Utilizadas)
...
Exemplo de uso de função e parâmetro tipados:

```php
function apresentasProduto(string $nome, float $preco): string{
    return "$nome custa R$ $preco";

}

$mensagem = apresentasProduto("Caderno", 25.00);
echo $mensagem;
...
function registraLog(string $mensagem): void{
    //apenas salvar em um arquivo de texto, não devolver nenhum variável
...
*Oque acontece dentro da função, fica dentro da função*. Uma variável criada fora não existe la dentro, e uma criada lá dentro morre quando a função acaba.
...
A forma mais segura e organizada é enviar os dados por **Parâmetro**. Assim, a função não precisa acessar diretamente variáveis globais:
...
> **Resumo**: variáveis protegem os dados internos da função; parâmetro são o caminho recomendado para evitar Erros e enviar Informações e `return` é usado para devolver um resultado ao códigp que chamou a função.
...
É aqui que o "BackEnd" começa de verdade. O Array Multidimencional é o formato como os Bancos de Dados chegam como resposta as solicitações feitas pela API.

**Exemplo de Aplicação de Array Multifimencional:**
...
// novo array, terá apenas os clientes que ativo por igual a true
```
...
$produtos = [
    ...
    ["id" =>2, "preco"=15.00, "setor"=> "ferramentas"],
...
#### A Anotomia de um Formulário HTML para BackEnd
...
**O 3 Pilares de Formulário** 

1. action="processa.php" -> O Destino: Define  qual script PHP no servidor receberá os dados.
2. method="POST" -> O Transporte: Define a vida de protocolo HTTP usada (GET ou POST).
...
Quando o Usuário clica no botão `type="submit"`, o navegador compila todas as informações dos campos preenchidos e dispara um pacote de comuncação padronizado pelo **Protocolo HTTP (Hypertext Transfer Protocol)** 
...
#### Como os Métodos Funcionam no PHP (`&_GET`, `$_POST`, `$_SERVER`)
...
- **_GET**: Aemazena dados passados pela URL via parâmetros de consulta (query string);
...
```php
$nome = $_POST["nome"];
// se escrever dessa forma, o código pode gerar um aviso de erro.

// A forma correta de esscrita é:
$nome = $_POST["nome"] ?? "";
...
Muitos desenvolvedores iniciantes acreditam que colocar atributos `required`, `type="email` ou `min=0` na <tag> do HTML é o suficiente para proteger o sistema. **Isso é Ilusão**. Sempre devemos fazer validações de dados no código BackEnd. As validações no BackEnd devem acontecer sempre antes do processamento de qualquer dado recebido pelo usuário.
...
>obs:use `htmlspecialchars()` ao exibir valor em HTML => converte caracteres especiais em entidades correspondente s em HTML, evitando que o código seja interpretado erradamente pelo navegador. É usado principalmente na segurança web, para evitar ataques Cross-Site-Scriptinf(XSS).


#### Presenvação de Estado em Formulários (*Sticky Form*)

A técnica do **Sticky Form** consiste em imprimir de volta o valor no atributo "value" do input, os dados que osusuário acab de diggitar, são devolvido aos inputs, caso ocorra algum erro de validação dedaos no envio.
...
> Nunca Confie no Usuário: Toda a entrada de dados vinda de fora do servidor é potencialmente maliciosa até que seja rigorosamente validada, sanitizada e codificada. 

Quando você disponibiliza um campo de texto em um site, qualquer pessoa conectada a internet pode digitar códigos maliciosos em vez de texto. Se o código BackEnd pega esse texto diretamente sem nenhum tratamento, a ordem de execução de código abrirá portas para a invasão devastadora do sistema. 
...
1. *Roubo de Sessão(Cookies Stealing)*: JavaScript injetado lê os cookies de autenticação da vítima (document.cookie) e os envia para o servidor de atacante, permitindo que ele faça login na conta da vítima sem precisar de senha.

2. *Desconfiguração de Site(defacement)*: Alterção visual do site, inserindo mensagens falsas, banners ofensivos ou formulários de login fraudulentos(phising interno).
...
Ex: Verificar se o e-mail possui `@` e domínio válido (`filter_var ($email, FILTER_VALIDADE_EMAIL)`).

2. **Sanitização**: Transforma o dado para adequa-lo ao formato desejado, removendo caracteres indesejados.

Ex: Remover espaços no início ao fim (`trim($nome)`).

3. **Escapamento/Codificação de Saída**: é o ato de converter caracteres especiais de linguagem HTML em suas respectivas
**Entidade HTML** no momento exato em que eles são impressos na tela.
...
string htmlspecialchars(
    string $string, 
    int $flags = ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTMLS,
...
- `ENT_HTMLS`: Aplica a tabela de entidade compatíveis com a especifição HTML5
- `UTF-8`: Garante que carateres de língua portuguesa como "ç", "ã", "é", sejam preservados sem corrupção.

**A função helper de Escapamento**

Para não digitar essa linha extensa em todas as partes de saída de texto os desenvolvedores profissionais criam uma função auxiliar curta:
...
```php
    ...
    <p>Comentários: <?= e($comentarioUsuario) ?></p>
...
**Da memória Volátil ao Banco de Dados**

Em sistemas corporativos de grande porte, arquivos planos (.txt 
.json) não oferecem a segurança, integridade, concorrência e velocidade necessárias para armazenamento de dados. Então é aqui que o **BackEnd encontram Banco de Dados Relacional**.
...
> Obs: Atomicidade, assegura que cada transação sejá unica. Consistência, respeite todas as regras, restrições e chaves definidas garantindo a validade da transação. Isolamento, transações de forma independente. Durabilidade, transações são confirmadas, garantindo a persistência permanente.
...
**Vantagens do uso PDO**

- **Portabilidae de Código**: Os métodos de conexão, consulta e transações são idênticos, independente do banco utilizado. Se o cliente migrar do banco PostgreSQL para outro SGBD, o programador apenas altera a string DSN de conexão, preservando toda a lógica de acesso já criada. 
...
- **Tratamento Orientado a Objetos com Exceptions**: Em vez de retornar códigos de rros, o PDO lança uma instância da classe especializada `PDOException`
...
```php 
$opcoes = [
    //1. flag: Lança execeções imediatamente quando ocorrer qualquer erro SQl
    ...
    //4. Limita 5 segundos para tentar a conexão com o servidor do BD
    PDO:: ATTR_TIMEOUT => 5
...
- PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION : por padrão o PDO pode falhar silenciosamente e retorna apenas `false`. Ao ativar o ERR_MODE força o PHP a dispara uma `PDO_Exception`, permitindo que o nosso código interprete qualquer erro em um bloco `try-catch`.
- PDO::ATTR_DEFAULT_FETCH_MODE => PDO::FETCH_ASSOC : por padrão e métodos `fetch()` retorna um array duplicado contendo índices numéricos `[0,1]` e associativos `["id", "código_maquina"]`. Definir `FETCH_ASSOC` reduz o consumo de mémoria RAM pela metade e entrega coleções limpas.
- PDO::ATTREMULATE_PREPARES => false: Garante que o PHP envie a consulta e os parâmetros separados diretamente para o planejador do BD processar, blindando a aplicação contra ataques sotisficados de `SQL_injection`
...
1. **Construtor Privado** (`private function_constructor`):
Impede que outros arquivos instaciem uma nova conexão
2. **Propriedades Estáticas Privadas** (`private static ?PDO $instancia = null`): Armazena a Conexão aberta. 
3. **Método de Acesso Estático Público** (`public static function obterConexão():PDO`): A conexão é Criada pelo método, se já existir uma conexão apenas devolve a conexão ja existente, sem a necessidade de criar uma nova.
